            ...
            <form onSubmit={validateAllFields}>
                ...
                <section className="grid gap-x-8 gap-y-6 sm:grid-cols-2">
                    <div className="space-y-1">
                        ...
                        <Text>{t("Select and customize your application's initial setup and development tools.")}</Text>
                    </div>
                ...
                <section className="grid gap-x-8 gap-y-6 sm:grid-cols-2">
                    <div className="space-y-1">
                        ...
                        <Text>{t('Add additional features like task scheduling, websockets, and job queue workers.')}</Text>
                    </div>
                    ...
                    <FieldGroup>
                        ...
                        <OptionalFeatures data={data} handleFeatureChange={handleFeatureChange} modifiedFields={modifiedFields} />
                    </FieldGroup>
                </section>

                <div className="mt-8 flex items-center justify-end gap-4">
                    {isLoading && <Text className="text-sm text-gray-500">{t('Analyse du package en cours...')}</Text>}
                    {validating && <Text className="text-sm text-gray-500">{t('Validation en cours...')}</Text>}
                    <Button type="submit" disabled={processing || validating || isLoading}>
                        {processing ? t('Génération...') : t('Generate project')}
                    </Button>
                </div>
            </form>
        </>
