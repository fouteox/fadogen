...
export default function Login({ status, canResetPassword }: { status?: string; canResetPassword: boolean }) {
    ...
    return (
        <>
            ...
            <form onSubmit={login} className={'mx-auto w-full sm:max-w-md'}>
                <Fieldset>
                    <Legend className={'text-center sm:text-xl'}>{t('Log in')}</Legend>
                    ...
                    <FieldGroup>
                        ...
                        <div className="flex flex-wrap justify-between gap-2">
                            <Headless.Field className="flex items-center gap-2">
                                <Switch name="remember" checked={data.remember} onChange={(checked) => setData('remember', checked)} />
                                <Label>{t('Remember me')}</Label>
                            </Headless.Field>

                            {canResetPassword && <TextLink href={request()}>{t('Forgot your password?')}</TextLink>}
                        </div>
