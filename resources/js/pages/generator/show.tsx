...
export default function Show({ template }: Props) {
    ...
    useEffect(() => {
        if (template.status === 'completed' || template.status === 'downloaded' || template.status === 'failed') {
            stop();
        }
            ...
            <div className="flex min-h-[60vh] items-center justify-center">
                <AnimatePresence mode="wait">
                    {template.status === 'pending' && (
                        <motion.div key="pending" className="space-y-8 text-center" initial="initial" animate="animate" exit="exit" variants={fadeIn}>
                            <div className="space-y-4">
                                <h3 className="text-2xl font-medium">{t('Preparing your environment')}</h3>
                                <Text>{t("We're setting up your template. It will only take a few seconds.")}</Text>
                            </div>
                        </motion.div>
                    )}

                    {template.status === 'failed' && (
                        <motion.div key="error" variants={fadeIn} initial="initial" animate="animate" exit="exit">
                            <Text>{t('Template generation failed')}</Text>
                        </motion.div>
                    )}

                    {(template.status === 'completed' || template.status === 'downloaded') && template.download_command && (
                        <motion.div
                            key="success"
                        ...
                        >
                            <div className="space-y-4 text-center">
                                ...
                                <h3 className="text-2xl font-medium">{t('Your template is ready!')}</h3>
                                <Text>{t('Copy and run the command below in a terminal to start your project.')}</Text>
                            </div>
                            ...
                            >
                                <Text>{t('This command will automatically configure your project with the settings you selected.')}</Text>
                            </motion.div>
                        </motion.div>
                    )}
                </AnimatePresence>
