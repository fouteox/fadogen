...
export function ErrorMessage({
    ...
    return (
        <Headless.Description
            data-slot={variant}
            {...props}
            className={clsx(className, 'text-base/6 data-disabled:opacity-50 sm:text-sm/6', variantStyles[variant])}
        />
    );
}

export function FieldsetInfoMessage({ className, ...props }: React.PropsWithChildren<{ className?: string }>) {
    return (
        <div
            data-slot="info"
            {...props}
            className={clsx(className, 'mt-3 text-base/6 text-blue-600 data-disabled:opacity-50 sm:text-sm/6 dark:text-blue-400')}
        />
    );
}

export function FieldsetErrorMessage({ className, ...props }: React.PropsWithChildren<{ className?: string }>) {
    return (
        <div
            data-slot="error"
            {...props}
            className={clsx(className, 'mt-3 text-base/6 text-red-600 data-disabled:opacity-50 sm:text-sm/6 dark:text-red-500')}
        />
    );
}

export const InfoMessage = (props: Omit<React.ComponentProps<typeof ErrorMessage>, 'variant'>) => <ErrorMessage variant="info" {...props} />;

export const SuccessMessage = (props: Omit<React.ComponentProps<typeof ErrorMessage>, 'variant'>) => <ErrorMessage variant="success" {...props} />;

export const WarningMessage = (props: Omit<React.ComponentProps<typeof ErrorMessage>, 'variant'>) => <ErrorMessage variant="warning" {...props} />;
